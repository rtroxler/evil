...
(evil-redirect-digit-argument evil-motion-state-map "_" 'evil-beginning-of-line)
...
(define-key evil-motion-state-map "0" 'evil-next-line-1-first-non-blank)
...
(define-key evil-read-key-map (kbd "ii") #'keyboard-quit)
...
(define-key evil-insert-state-map "i" #'cofi/maybe-exit)

;; Shit to add ii as an esc 
(evil-define-command cofi/maybe-exit ()
  :repeat change
  (interactive)
  (let ((modified (buffer-modified-p)))
    (insert "i")
    (let ((evt (read-event (format "Insert %c to exit insert state" ?i)
               nil 0.5)))
      (cond
       ((null evt) (message ""))
       ((and (integerp evt) (char-equal evt ?i))
    (delete-char -1)
